...
export default class FakeBackend {
  ...
  static getFakeEventsResult(url: string,
                                   year: number,
                                   cityId: any,
                                   streetId: any,
                                   abfuhrbezirkId: any,
                                   papier: any,
                                   abfuhrbezirkpapier: any) : Event[] {
    ...
    if(abfuhrbezirkpapier !== '0' || cityId===-1){
      if(papier===UrlHelper.getProviderSearchField(Provider.PAMO) || cityId===-1){
        fakeEvents.push(new Event("10.02."+year,"Altpapier", Provider.PAMO))
      }
      if(papier===UrlHelper.getProviderSearchField(Provider.SIEMER) || cityId===-1){
        fakeEvents.push(new Event("19.02."+year,"Altpapier", Provider.SIEMER))
      }
    }
  ...
  static async returnFakeEventsRequest(url: string,
                                       year: number,
                                       cityId: any,
                                       streetId: any,
                                       abfuhrbezirkId: any,
    ...
    let fakeEvents = FakeBackend.getFakeEventsResult(url, year, cityId, streetId, abfuhrbezirkId, papier, abfuhrbezirkpapier);
    return {"text" : () => {
        ...
        for(let event of fakeEvents){
          ...
          html += '<span class="month_termin">'+dateString+' : '+label+'</span>'
        }
        return html;
      }};
  }
  ...
  static async fakeFetch(
    ...
    if(url===UrlHelper.CALENDAR_URL){
      return FakeBackend.returnFakeEventsRequest(url, year, cityId, streetId, abfuhrbezirkId, papier, abfuhrbezirkpapier);
    }
