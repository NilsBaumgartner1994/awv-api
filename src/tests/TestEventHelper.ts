...
test('Test isRestabfall', async () => {
  const event = new Event("01.01.2021", EventLabels.RESTABFALL, Provider.VECHTA)
  expect(EventHelper.isRestabfall(event)).toBeTruthy();
});

test('Test isAltkleider', async () => {
  const event = new Event("01.01.2021", EventLabels.ALTKLEIDER, Provider.VECHTA)
...
test('Test isGelbeTonne', async () => {
  const event = new Event("01.01.2021", EventLabels.GELBE_TONNE, Provider.VECHTA)
  expect(EventHelper.isGelbeTonne(event)).toBeTruthy();
});

test('Test isSchadstoff', async () => {
  const event = new Event("01.01.2021", EventLabels.SCHADSTOFF, Provider.VECHTA)
